...
// set the photoshop ruler pref to pixels
...
function saveFile( docRef, fileNameBody, exportInfo) {
  ...
  switch (exportInfo.fileType) {
    ...
    case jpegIndex:
      ...
      fileToSave = new File(exportInfo.destination + '/' + fileNameBody + '.jpg');
...
function exportChildren(dupDoc, exportInfo) {
  ...
  for(artIdx; artIdx < artLen; artIdx += 1) {
    ...
    layerName = webStr(dupDoc.artLayers[artIdx].name);
 ...
 */
function setExportHeader(psd, exportInfo) {
...
function writeCss() {
  ...
  cssOut.writeln('    height: ' + hdr.psdHeight + 'px;');
  ...
  for (idx; idx >= 0; idx -= 1) {
    ...
    cssOut.writeln('    height: ' + images[idx].height + 'px;');
    cssOut.writeln('    width:  ' + images[idx].width + 'px;');
    cssOut.writeln('}');
  }
  cssOut.close();
}
